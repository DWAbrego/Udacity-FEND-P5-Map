...
viewModel.initializeVm = function(model) { 
	...
	self.placesArrayObs = ko.observableArray(this.model.getLocations());
	...
	self.SearchResults1 = ko.computed(function () {
		console.log(self.model.locations[0].marker);

		for (i1 = 0; i1 < this.model.locations.length; i1++) {
		  self.model.locations[i1].marker.setVisible(false);
		}
		return ko.utils.arrayFilter(self.placesArrayObs(), function (item) {
		  var filter = self.filterText().toLowerCase();
		  for (i1 = 0; i1 < this.model.locations.length; i1++) {
			if (stringStartsWith(this.model.locations[i1].name1.toLowerCase(), filter)) {
				this.model.locations[i1].marker.setVisible(true);
			}
		  }
